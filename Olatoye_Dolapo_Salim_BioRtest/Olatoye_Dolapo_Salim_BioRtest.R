...
## EXPORT
# Using for loop to export files
# Create list of the df
export_list <- list(
  "clean_master_data.csv" = dat,
  "model_treatment.csv" = models_treat_only,
  "model_temperature.csv" = models_temp,
  "model_ndvi.csv" = models_ndvi,
  "model_combined.csv" = models_combined,
  "model_comparison.csv" = comparison
)
#?names()

for (i in names(export_list)) {
  write_csv(export_list[[i]], file.path(output_dir, i))
  message("Export complete ", i)
}
